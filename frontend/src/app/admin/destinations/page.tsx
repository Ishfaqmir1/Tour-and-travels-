              ...
              <tbody>
                {items.map(item => (
                  <tr key={item.id}>
                    ...
                    <td>{item.slug}</td>
                    <td>{item.image ? <span style={{ opacity: 0.6 }}>✓ attached</span> : <span style={{ opacity: 0.4 }}>—</span>}</td>
                    <td>
          ...
          <motion.div className="admin-modal-overlay" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setShowModal(false)}>
            <motion.div className="admin-modal" initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} onClick={e => e.stopPropagation()}>
              ...
              <div className="admin-form">
                ...
                <div className="admin-form-group"><label>Image URL</label><input value={editing?.image || ''} onChange={e => setEditing(prev => ({ ...prev!, image: e.target.value }))} /></div>
